...
namespace negocio
{
    public class NegocioImagen
    {
        ...
        public List<Imagen> listFMovie(int id)
        {
            List<Imagen> lista = new List<Imagen>();
            AccesoDatos datos = new AccesoDatos();
            try
            {
                datos.setearConsulta("select id, ImageUrl, movie_id from images where movie_id = @id");
                datos.setearParametro("@id", id);
                datos.ejecutarLectura();
                ...
                while (datos.Lector.Read())
                {
                    ...
                    imagen.Id = datos.Lector.GetInt32(0);
                    imagen.Url = (string)datos.Lector["ImageUrl"] != null ? (string)datos.Lector["ImageUrl"] : "https://encrypted-tbn2.gstatic.com/images?q=tbn:ANd9GcQogFO9C5CpJWSO1PoItMbYVr968uCyHzKPZ8LD26z0J46BI4Vg";
                    imagen.Movie_Id = datos.Lector.GetInt32(2);
                    lista.Add(imagen);
                }
                if (lista.Count == 0)
                {
        ...
        public List<Imagen> listFSeries(int id)
        {
            List<Imagen> lista = new List<Imagen>();
            AccesoDatos datos = new AccesoDatos();
            try
            {
                datos.setearConsulta("select id, ImageUrl, series_id from images where series_id = @id");
                datos.setearParametro("@id", id);
                datos.ejecutarLectura();
